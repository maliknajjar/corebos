...
class CustomView extends CRMEntity {
	...
	public function getCustomViewByCvid($cvid) {
		...
		$log->fatal('PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP');
		$log->fatal($permissions);
	...
	public function isPermittedCustomView($record_id, $action, $module) {
		...
		$cp = cbCVManagement::getPermission($record_id, $current_user->id);		// we need to use this function
		// global $log;
		// $log->fatal('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
		// $log->fatal($cp);
		// $log->fatal($action);
		// $log->fatal($record_id);
		// $log->fatal($current_user->id);
		$cp = array('C' => '0', 'R' => '0', 'U' => '0', 'D' => '0', 'A' => '0', );
		$permission = 'no';
		if ($action == "CustomView" && $cp['R']) {
			$permission = 'yes';
		...
		// $log->fatal('returned permission is:');
